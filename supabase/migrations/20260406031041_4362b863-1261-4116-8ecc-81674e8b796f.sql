...
AS $function$
  ...
  candidate_ids AS (
    ...
    -- Title LIKE match on main query
    SELECT j.id
    FROM public.jobs j CROSS JOIN flags f
    WHERE f.has_query
      AND j.is_published = true AND j.is_archived = false
    ...
    -- Company LIKE match on main query
    SELECT j.id
    FROM public.jobs j CROSS JOIN flags f
    WHERE f.has_query
      AND j.is_published = true AND j.is_archived = false
    ...
    -- Expanded terms: title LIKE match (NEW)
    SELECT j.id
    FROM public.jobs j CROSS JOIN flags f
    JOIN LATERAL unnest(f.clean_expanded) AS et(term) ON f.has_expanded
    WHERE j.is_published = true AND j.is_archived = false
  ...
  SELECT
    ...
    (
      CASE
        -- Exact title match on main query
...
-- Also update count_search_jobs to include expanded-term title matches
