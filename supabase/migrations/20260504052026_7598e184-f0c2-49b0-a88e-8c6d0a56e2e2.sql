...
BEGIN
  ...
  flags AS (
    SELECT s.*,
      ...
      -- significant title tokens from the query (length > 2, drops short connector words and roman numerals like "ii")
      COALESCE(ARRAY(
  ...
  candidate_ids AS (
    ...
    UNION
    -- FTS base, but ONLY when title contains ALL significant query tokens
    SELECT j2.id FROM public.jobs j2 CROSS JOIN flags f2
    WHERE f2.has_query AND j2.is_published=true AND j2.is_archived=false AND j2.is_direct_apply=true
      AND j2.deleted_at IS NULL AND j2.posted_date>=f2.cutoff_date
      AND j2.external_apply_link NOT ILIKE '%linkedin.com%'
      AND j2.search_vector@@f2.base_tsquery
      AND COALESCE(array_length(f2.query_tokens,1),0) > 0
      AND NOT EXISTS (
        SELECT 1 FROM unnest(f2.query_tokens) AS qt(tok)
        WHERE lower(j2.title) NOT LIKE '%'||qt.tok||'%'
      )
    UNION
    -- Expanded FTS, gated similarly: title must contain all query tokens (when a query exists)
    SELECT j2.id FROM public.jobs j2 CROSS JOIN flags f2
    JOIN LATERAL unnest(f2.clean_expanded) AS et(term) ON f2.has_expanded
    WHERE j2.is_published=true AND j2.is_archived=false AND j2.is_direct_apply=true
      AND j2.deleted_at IS NULL AND j2.posted_date>=f2.cutoff_date
      ...
      AND (
        NOT f2.has_query
        OR (
          COALESCE(array_length(f2.query_tokens,1),0) > 0
          AND NOT EXISTS (
            SELECT 1 FROM unnest(f2.query_tokens) AS qt(tok)
            WHERE lower(j2.title) NOT LIKE '%'||qt.tok||'%'
          )
  ...
  tiered AS (
    SELECT r.*,
      CASE
        ...
        WHEN r.rank >= 75.0  THEN 4
        WHEN r.rank >= 60.0  THEN 3
        WHEN r.rank >= 40.0  THEN 2
        WHEN r.rank >= 25.0  THEN 1
        ELSE 0
