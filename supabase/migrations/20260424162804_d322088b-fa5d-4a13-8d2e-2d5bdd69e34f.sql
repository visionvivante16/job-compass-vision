...
BEGIN
  ...
  tiered AS (
    SELECT r.*,
      ...
      -- No-query browse: single tier so pure recency applies.
      CASE
        WHEN NOT r._has_query THEN 0
        WHEN r.rank >= 100.0 THEN 5  -- exact title
        WHEN r.rank >= 85.0  THEN 4  -- title prefix
        WHEN r.rank >= 60.0  THEN 3  -- title contains (query or expanded)
