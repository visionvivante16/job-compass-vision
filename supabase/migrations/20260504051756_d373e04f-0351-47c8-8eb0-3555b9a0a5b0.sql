...
BEGIN
  ...
  flags AS (
    SELECT s.*,
      ...
      -- significant title tokens from the query (length > 2, drops short connector words)
      COALESCE(ARRAY(
        SELECT DISTINCT lower(tok)
        FROM regexp_split_to_table(COALESCE(lower(NULLIF(trim(s.clean_query),'')), ''), '[^a-z0-9]+') AS tok
        WHERE length(tok) > 2
      ), ARRAY[]::text[]) AS query_tokens
    FROM search_scope s
  ),
  candidate_ids AS (
    -- No query: recent published direct-apply jobs
    ...
    UNION
    -- Direct title contains the query
    SELECT j2.id FROM public.jobs j2 CROSS JOIN flags f2
    ...
    UNION
    -- FTS base, but ONLY when title also contains at least one significant query token
    SELECT j2.id FROM public.jobs j2 CROSS JOIN flags f2
    WHERE f2.has_query AND j2.is_published=true AND j2.is_archived=false AND j2.is_direct_apply=true
      AND j2.deleted_at IS NULL AND j2.posted_date>=f2.cutoff_date
      AND j2.external_apply_link NOT ILIKE '%linkedin.com%'
      ...
      AND EXISTS (
        SELECT 1 FROM unnest(f2.query_tokens) AS qt(tok)
        WHERE lower(j2.title) LIKE '%'||qt.tok||'%'
      )
    UNION
    -- Expanded FTS, also gated by title-token overlap with the query
    SELECT j2.id FROM public.jobs j2 CROSS JOIN flags f2
    JOIN LATERAL unnest(f2.clean_expanded) AS et(term) ON f2.has_expanded
    WHERE j2.is_published=true AND j2.is_archived=false AND j2.is_direct_apply=true
      AND j2.deleted_at IS NULL AND j2.posted_date>=f2.cutoff_date
  ...
  tiered AS (
    SELECT r.*,
      CASE
        WHEN NOT r._has_query THEN 0
        WHEN r.rank >= 75.0  THEN 4  -- direct title match (exact / prefix / contains)
        WHEN r.rank >= 60.0  THEN 3  -- expanded title contains (related role)
        WHEN r.rank >= 40.0  THEN 2  -- FTS on description/skills (now title-gated)
        WHEN r.rank >= 25.0  THEN 1  -- expanded FTS (title-gated)
        ELSE 0                       -- company-only / weak
      END AS relevance_tier
    FROM ranked_jobs r
  )
  SELECT t.id, t.title, t.company, t.company_logo, t.location, t.description, t.skills,
    t.external_apply_link, t.is_published, t.is_reviewing, t.salary_range, t.employment_type,
    t.experience_years, t.posted_date, t.created_at, t.updated_at, t.is_archived, t.rank
  FROM tiered t
  ORDER BY t.relevance_tier DESC, t.posted_date DESC, t.rank DESC
