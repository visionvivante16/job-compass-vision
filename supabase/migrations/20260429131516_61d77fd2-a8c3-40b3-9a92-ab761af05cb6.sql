...
BEGIN
  ...
  tiered AS (
    SELECT r.*,
      -- Group jobs into relevance tiers; within a tier, recency wins.
      -- Title matches (exact / prefix / contains / expanded-contains) all collapse
      -- into one tier so the freshest title-matching job always wins regardless of
      -- whether it's an exact or partial match. Body / company matches stay below.
      CASE
        WHEN NOT r._has_query THEN 0
        WHEN r.rank >= 60.0  THEN 3  -- any title match (exact, prefix, contains, expanded)
        WHEN r.rank >= 40.0  THEN 2  -- FTS on description/skills
        WHEN r.rank >= 25.0  THEN 1  -- expanded FTS
